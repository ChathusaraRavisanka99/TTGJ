...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const content = await getPageContent("seasonal", DEFAULT_SEASONAL_CONTENT);
  return { title: visibility === "COMING_SOON" ? "Coming Soon" : content.heading };
}
...
export default async function PromotionsPage() {
  ...
  const content = await getPageContent("seasonal", DEFAULT_SEASONAL_CONTENT);
  const theme = SEASONAL_THEMES[content.theme] ?? SEASONAL_THEMES.autumn;
    ...
    <div className={`relative flex min-h-[85dvh] items-center justify-center overflow-hidden ${theme.backgroundClass}`}>
      ...
      <div className="relative z-10 mx-auto max-w-xl px-5 py-24 text-center sm:px-8">
        <p className={`text-xs uppercase tracking-[0.35em] ${theme.kickerClass}`}>
          {isComingSoon ? "Coming Soon" : content.kicker}
        </p>
        <h1 className={`mt-4 font-serif text-4xl leading-tight sm:text-5xl ${theme.headingClass}`}>
          {isComingSoon ? "Something seasonal is on its way." : content.heading}
        </h1>
        <p className={`mt-5 ${theme.bodyClass}`}>
          {isComingSoon
            ? "Check back soon for a limited-time collection."
            : content.body}
        </p>
        {!isComingSoon && (
          <div className="mt-8">
            <LinkButton href={content.ctaHref} variant="gold" size="lg">
              {content.ctaLabel}
            </LinkButton>
          </div>
        )}
