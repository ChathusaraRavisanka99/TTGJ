...
// separately opted into public pricing (showPrice) and that price is
// actually higher than the promo one.
export function ProductPrice({ price, showPrice, promotion }: ProductPriceProps) {
  if (promotion) {
    const showRegular = showPrice && price != null && price > promotion.promoPrice;
    return (
      <div className="mt-2 flex flex-wrap items-baseline gap-3">
        <p className="font-serif text-2xl text-gold">{formatPrice(promotion.promoPrice)}</p>
        {showRegular && <p className="text-lg text-charcoal/40 line-through">{formatPrice(price!)}</p>}
  ...
  if (showPrice && price != null) {
    return <p className="mt-2 font-serif text-2xl text-gold">{formatPrice(price)}</p>;
  }
